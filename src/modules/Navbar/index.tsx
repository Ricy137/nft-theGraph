...
const NavLink: React.FC<ComponentProps<typeof Link> & { curPath: boolean }> = ({ to, children, curPath }) => (
  <li className={cx('navbar-link relative flex items-center px-14px h-full overflow-hidden',curPath&&'border-b-1px border-b-solid border-#ffffff')}>
...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="px-20px flex justify-center h-61px bg-#6960FA border-b-1px border-#FEFEFE border-opacity-15 ">
      <Mobile open={isMobileMenuOpen} curPath={curPath === '/' ? '/mint' : curPath} />
      <nav className={cx('max-w-1920px mx-auto flex items-center w-full h-60px px-32px bg-#6960FA z-100')}>
