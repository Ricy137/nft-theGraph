...
export const useShowToast = () => {
  ...
  const showToast = useCallback(
    (param: Omit<Toast, 'id'> & { key?: string; duration?: number }) => {
      if (param.key && toasts.find((item: Toast) => item.key && item.key === param.key)) return;
...
export const ToastRender: React.FC = () => {
  ...
  return (
    <div className="fixed left-0 top-[5%] right-0 flex flex-col justify-center items-center gap-[12px] z-1000">
      {toasts.map(({ content, type, id }) => (
        ...
        >
          {type === 'success' ? <CheckedIcon className="w-[14px] h-[40px]" /> : <FailedIcon className="w-[14px] h-[40px]" />}
